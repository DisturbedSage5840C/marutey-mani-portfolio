...
function createGrid(count: number): Float32Array {
  ...
  for (let i = 0; i < count; i += 1) {
    out[i * 3]     = ((i % side) - side / 2) * 0.08;
    out[i * 3 + 1] = (Math.random() - 0.5) * 0.12;
    out[i * 3 + 2] = (Math.floor(i / side) - side / 2) * 0.08;
  }
  return out;
}
...
function FloatingTorusKnot() {
  ...
  useFrame(({ clock }) => {
    ...
    ref.current.position.x = Math.sin(t * 0.17) * 1.4 - 2.8;
    ref.current.position.y = Math.cos(t * 0.11) * 0.6 + 0.5;
  });
  return (
    <mesh ref={ref} position={[-2.8, 0.5, -2]}>
      <torusKnotGeometry args={[0.38, 0.12, 96, 12, 2, 3]} />
...
function FloatingIcosahedron() {
  ...
  useFrame(({ clock }) => {
    ...
    ref.current.position.x = Math.cos(t * 0.13) * 1.2 + 2.6;
    ref.current.position.y = Math.sin(t * 0.09) * 0.7 - 0.4;
  });
  return (
    <mesh ref={ref} position={[2.6, -0.4, -1.5]}>
...
function FloatingOctahedron() {
  ...
  useFrame(({ clock }) => {
    ...
    ref.current.position.y = Math.cos(t * 0.14) * 0.9 - 2.2;
  });
  return (
    <mesh ref={ref} position={[0, -2.2, -1]}>
      <octahedronGeometry args={[0.32]} />
      <meshBasicMaterial color="#b8a0f0" wireframe opacity={0.2} transparent />
    </mesh>
  );
}
